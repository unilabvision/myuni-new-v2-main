// components/CourseErrorState.tsx
"use client";
...
const CourseErrorState: React.FC<CourseErrorStateProps> = ({ 
  ...
  const errorConfig = {
    not_found: {
      ...
      title: error === 'Course not found' ? texts.courseNotFound : 'Sayfa Bulunamadı',
      description: error === 'Course not found' 
        ? texts.courseNotFoundDesc 
        : 'Aradığınız sayfa mevcut değil veya taşınmış olabilir.',
      suggestions: [
        'URL\'yi kontrol edin',
        'Ana sayfaya dönün',
        'Arama özelliğini kullanın',
        'Popüler kursları inceleyin'
      ]
    },
    network: {
      icon: Wifi,
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-neutral-900 flex items-center justify-center">
      <div className="max-w-2xl mx-auto px-6 py-12 text-center">
          ...
          <div className="mt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <Link
                  key={index}
                  href={`/${locale}/${courseType}/${course.slug}`}
                  ...
                  <div className="flex items-center mt-2">
                    <div className="flex items-center space-x-1">
                      {[...Array(5)].map((_, i) => (
                        <svg
                          key={i}
                          className={`w-3 h-3 ${i < Math.floor(course.rating) ? 'text-yellow-400' : 'text-neutral-300'}`}
                          fill="currentColor"
                          viewBox="0 0 20 20"
                        >
        ...
        {/* Back to Course List (Mobile-friendly) */}
