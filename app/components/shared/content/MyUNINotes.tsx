...
interface MyUNINotesProps {
  contentId: string; // lessonId or sectionId (for content fetching)
  progressId?: string; // Optional: separate ID for progress tracking (lesson.id for per-lesson tracking)
  userId?: string;
...
export function MyUNINotes({
  ...
  // Use progressId for progress tracking, contentId for content fetching
  const progressTrackingId = progressId || contentId;
  ...
  const handleProgressSave = useCallback(async (timeSpent: number, forceComplete: boolean = false) => {
    ...
    try {
      ...
      console.log('Saving progress for type:', type, 'progressTrackingId:', progressTrackingId, 'contentId:', contentId, 'progressData:', progressData);

      // Save progress based on content type
      // For courses: use contentId (lessonId)
      // For events: use progressTrackingId (lesson.id) — NOT contentId (sectionId) to prevent lesson conflicts
      if (type === 'course') {
        await updateUserProgress(userId, contentId, progressData);
      } else {
        await updateUserEventProgress(userId, progressTrackingId, progressData);
      }
  ...
  }, [userId, contentId, progressTrackingId, currentNote, userProgress, userNote, onComplete, type]);
  ...
  const loadUserProgress = useCallback(async () => {
    ...
    try {
      console.log('Loading user progress for type:', type, 'progressTrackingId:', progressTrackingId, 'contentId:', contentId);

      let progress;
      if (type === 'course') {
        progress = await getUserLessonProgress(userId, contentId);
      } else {
        // For events: use progressTrackingId (lesson.id) — NOT contentId (sectionId)
        // This ensures each lesson has its own individual progress record
        progress = await getUserEventLessonProgress(userId, progressTrackingId);
      }
  ...
  }, [userId, contentId, progressTrackingId, currentNote, type]);

  const calculateEstimatedReadingTime = (content: string): number => {
    if (!content || content.trim() === '') {
      return 15; // Automatically set 15 seconds for PDFs or materials without text body
    }
    const wordsPerMinute = 200;
    const wordCount = content.split(/\s+/).length;
    return Math.max((wordCount / wordsPerMinute) * 60, 15); // Minimum 15 seconds
  };
    ...
    if (isActive && currentNote && !userProgress.is_completed && userId) {
      interval = setInterval(() => {
        setReadingTime(prev => {
          ...
          // Auto-save progress and check for completion every 5 seconds
          if (newTime % 5 === 0) {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        {/* User Note Editor */}
        {showUserNoteEditor && (
          <div className={`p-4 border-b ${type === 'course'
            ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800'
            : 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800'
            }`}>
            <h4 className={`text-sm font-medium mb-2 ${type === 'course'
            ...
            <textarea
              ...
              className={`w-full h-24 p-3 border rounded resize-none text-sm bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100 placeholder-neutral-500 dark:placeholder-neutral-400 ${type === 'course'
            ...
            <div className="flex items-center justify-end space-x-2 mt-2">
              ...
              <button
                ...
                className={`px-3 py-1 text-white text-sm rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${type === 'course'
        ...
        <div className="p-4 bg-white dark:bg-neutral-800">
          ...
          {currentNote.file_url && (
            <div className="mt-8 p-0 rounded-lg overflow-hidden border border-neutral-200 dark:border-neutral-700 shadow-sm bg-neutral-50 dark:bg-neutral-800">
              <div className="flex items-center justify-between p-4 bg-white dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-700">
                <h4 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100 flex items-center space-x-2">
                  <FileText className="w-4 h-4 text-neutral-500" />
                  <span>Önizleme ({getContentTypeLabel(currentNote.content_type)})</span>
                </h4>
              </div>


              {currentNote.file_url.toLowerCase().includes('.pdf') ? (
                <div className="w-full h-[75vh] min-h-[600px] bg-neutral-100 dark:bg-neutral-900 relative">
                  <div className="absolute inset-0 flex items-center justify-center text-neutral-400 text-sm -z-10">
                    PDF Yükleniyor...
                  </div>
                  <iframe
                    src={`${currentNote.file_url}#toolbar=0&navpanes=0&scrollbar=1`}
                ...
                <div className="p-6 text-center text-neutral-500 dark:text-neutral-400">
                  <p>Bu belge türü sayfa içinde önizlenemiyor. Dosyayı indirip görüntüleyebilirsiniz.</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="space-y-2">
              {notes.map((note) => (
                <button
                  ...
                  className={`w-full flex items-center space-x-3 p-3 rounded text-left transition-colors ${currentNote.id === note.id
