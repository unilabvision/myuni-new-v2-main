// components/shared/AIChatSidebar.tsx
"use client";
...
interface ChatRequest {
  ...
  itemType?: 'course' | 'event'; // Yeni alan
  itemId?: string; // courseId veya eventId
  itemTitle?: string; // course/event title
}
...
interface AIChatSidebarProps {
  ...
  type?: 'course' | 'event'; // Yeni prop: kurs mu etkinlik mi
  itemId?: string; // courseId veya eventId
  itemTitle?: string; // course/event title
}
...
export default function AIChatSidebar({
    ...
    const basePrompts = [
      {
        ...
        text: type === 'course' ? "Bu dersin ana konularını açıkla" : "Bu etkinliğin ana konularını açıkla",
      ...
      },
      {
        ...
        text: type === 'course' ? "Bu dersten sonra ne yapmalıyım?" : "Bu etkinlikten sonra ne yapmalıyım?",
  ...
  const suggestedPrompts = React.useMemo(() => getSuggestedPrompts(), [type]);
  ...
  // Mobile scroll and touch management
  useEffect(() => {
  ...
  const handleSendMessage = async (messageText?: string) => {
    ...
    try {
      const requestBody: ChatRequest = {
        ...
        conversationHistory: messages.slice(-4), // Last 4 messages
        itemType: type,
        itemId: itemId,
        itemTitle: itemTitle
      };

      console.log('Enhanced AI request being sent:', { 
      ...
      // Mark if lesson context was used
  ...
  const renderMessageContent = (content: string) => {
    // Detect code blocks - more reliable regex
    ...
    // If no code blocks, return all content as text
    if (parts.length === 0) {
      return [{
        type: 'text',
        content: content
  ...
  const getItemIcon = () => {
    return type === 'course' ? <BookOpen className="w-4 h-4 text-gray-600 dark:text-neutral-400" /> : <Calendar className="w-4 h-4 text-gray-600 dark:text-neutral-400" />;
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex-shrink-0 p-6 border-b border-gray-100 dark:border-neutral-800">
                  ...
                  {lessonContextAvailable && (
                    <div className="flex items-center space-x-1 px-2 py-1 bg-green-100 dark:bg-green-900/30 rounded-full">
                      ...
                      <span className="text-xs font-medium text-green-700 dark:text-green-300">
                        {type === 'course' ? 'Ders İçeriği' : 'Etkinlik İçeriği'}
                      </span>
                    </div>
                  )}
                </div>
          ...
          {/* Enhanced lesson/event context display */}
          {selectedLesson && (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center space-x-2 mt-1">
                    <p className="text-xs text-gray-600 dark:text-neutral-400 font-medium">
                      {type === 'course' ? 'Aktif Ders' : 'Aktif İçerik'}
                    </p>
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                  {selectedLesson 
                    ? `Bu ${type === 'course' ? 'Ders' : 'İçerik'} İçin Öneriler`
                    : 'Hızlı Başlangıç'
                  }
                </span>
              </div>
                ...
                <div className="text-center space-y-3">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                      {type === 'course' ? 'Ders İçeriği Hazır' : 'Etkinlik İçeriği Hazır'}
                    </p>
                    <p className="text-xs text-gray-600 dark:text-neutral-400 max-w-48">
                      {type === 'course' 
                        ...
                        : 'Bu etkinliğin içeriğini biliyorum. Size daha detaylı yardım edebilirim.'
                      }
                    </p>
                  </div>
                </div>
                      ...
                      <div className="bg-gray-100 dark:bg-neutral-800 px-5 py-3 border border-gray-200 dark:border-neutral-700">
                        <div className="flex items-center space-x-3">
                          ...
                          <span className="text-xs text-gray-600 dark:text-neutral-400 font-bold">
                            {lessonContextAvailable 
                              ? `${type === 'course' ? 'Ders' : 'Etkinlik'} içeriğini analiz ediyor...`
                              : 'Düşünüyor...'
                            }
                          </span>
                        </div>
        ...
        <div className="flex-shrink-0 p-5 border-t border-gray-100 dark:border-neutral-800">
          <div className="relative">
            <textarea
              ...
              placeholder={
                selectedLesson 
                  ? `${selectedLesson.title} hakkında sorun...`
                  : itemTitle
                  ? `${itemTitle} hakkında sorun...`
                  : "Mesajınızı yazın..."
              }
              disabled={isLoading}
          ...
          {/* Enhanced keyboard shortcuts and retry button */}
