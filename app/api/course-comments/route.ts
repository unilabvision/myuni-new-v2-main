...
// Helper: anonymize full name -> initials like "Y.P."
...
// GET: list comments by courseId or get average rating
...
// POST: create a comment if user enrolled to this course
export async function POST(request: Request) {
  try {
    ...
    const { courseId, content, rating, isAnonymous } = body as { courseId?: string; content?: string; rating?: number; isAnonymous?: boolean };

    if (!courseId || !content || content.trim().length < 3) {
    ...
    if (!enrollment) {
      return NextResponse.json({ error: 'Yorum yapmak için kursa kayıtlı olmalısınız' }, { status: 403 });
    }
    ...
    try {
      ...
      if (!courseError && courseData) {
        ...
        try {
          await sendCommentNotifications(commentData, courseInfo, locale);
      ...
      console.error('Comment notification setup error:', notificationError);
      // Don't fail the comment creation if notification setup fails
    }

    return NextResponse.json({ comment: data }, { status: 201 });
  } catch (err) {
    console.error('POST /course-comments error', err);
...
export async function PUT(request: Request) {
  try {
    ...
    console.error('PUT /course-comments error', err);
...
export async function DELETE(request: Request) {
  try {
    ...
    console.error('DELETE /course-comments error', err);
